...
public class PetitionStaffImportantMonitorAddMatterWTTypeActivity01 extends PetitionStaffBaseActivity implements View.OnClickListener {
    ...
    protected void initView() {
        ...
        mBinding= DataBindingUtil.setContentView(this, R.layout.petition_staff_activity_request_add_address1);

        list=GsonUtil.fromJson(getSharedPreferences("LongSp",MODE_PRIVATE).getString("AllType",""),new TypeToken<List<RAllAddress>>(){}.getType());
        ...
        mBinding.listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (data1.size()==0) {
                    for (int i=0;i<list.size();i++) {
                        if (list.get(i).getDM().equals(getIntent().getExtras().getString("DM"))) {
                            editor.putString(getString(R.string.petition_staff_sp_JYLXY),list.get(i).getMC());
                            editor.putString(getString(R.string.petition_staff_sp_JYLXYDM),list.get(i).getDM());
                        }
                    }
                    editor.putString(getString(R.string.petition_staff_JYLXE),data.get(position).getMC());
                    editor.putString(getString(R.string.petition_staff_JYLXEDM),data.get(position).getDM().toString());

                    editor.putString(getString(R.string.petition_staff_JYLXS),"");
                    editor.putString(getString(R.string.petition_staff_JYLXSDM),"");

                    editor.commit();
                    PetitionStaffImportantMonitorAddMatterWTTypeActivity.instance.finish();
                    PetitionStaffImportantMonitorAddMatterWTTypeActivity01.this.finish();
                }
                mAdapter1=new AddAddressAdapter(data1,PetitionStaffImportantMonitorAddMatterWTTypeActivity01.this);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                for (int i=0;i<list1.size();i++) {
                    if (data1.get(position).getFJDM().equals(list1.get(i).getDM())) {
                        editor.putString(getString(R.string.petition_staff_JYLXE),list1.get(i).getMC());
                        editor.putString(getString(R.string.petition_staff_JYLXEDM),list1.get(i).getDM());
                    }
                }

                editor.putString(getString(R.string.petition_staff_JYLXS),data1.get(position).getMC());
                editor.putString(getString(R.string.petition_staff_JYLXSDM),data1.get(position).getDM().toString());
                editor.commit();
