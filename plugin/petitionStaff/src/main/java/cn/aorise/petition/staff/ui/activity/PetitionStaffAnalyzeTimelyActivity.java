...
import cn.aorise.petition.staff.module.network.entity.request.TAnalyze02;
...
public class PetitionStaffAnalyzeTimelyActivity extends PetitionStaffBaseActivity implements View.OnClickListener {
    private static final String TAG = PetitionStaffLoginActivity.class.getSimpleName();
    private PetitionStaffActivityAnalyzewulvBinding mBinding;

    private BarChart barChart;// 柱形图控件
    private BarData data;// x轴数据
    private BarDataSet dataSet;// Y轴数据
    private String[] st = {"总数", "及时受理数", "未及时受理数"};//柱状图
    ...
    private String[] x = new String[]{"及时受理数", "未及时受理数"}; //饼状图
    private Float[] y;
    ...
    protected void initView() {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.petition_staff_activity_analyzewulv);
        ...
        mBinding.txtTitle.setText("五率统计");
    ...
    private void GetAnalyze(String dataStart, String dataEnd, String type,String xingshi) {
        ...
        Subscription subscription = PetitionStaffApiService.Factory.create().GetJishiShouliLv(GsonUtil.toJson(t))
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(Utils.mockSubscriber(this, Mock.PETITION_LOGIN,
                        new TypeToken<APIResult<RAnalyze>>() {
                            ...
                            public void onNext(APIResult<List<Float>> rAnalyzeAPIResult) {
                                ...
                                try {
                                    ...
                                    MpChartAndroidUtil.showBarChart1(PetitionStaffAnalyzeTimelyActivity.this, mBinding.barchart, barData);
                                } catch (Exception e) {

                                }
                                if (rAnalyzeAPIResult.getData().get(0)!=null) {
                                    mBinding.txt1.setText("信访部门及时受理率："+rAnalyzeAPIResult.getData().get(0)+"%");
                                } else {
                                    mBinding.txt1.setText("信访部门及时受理率："+"0.0%");
                                }
                                if (rAnalyzeAPIResult.getData().get(1)!=null) {
                                    mBinding.txt2.setText("责任部门及时受理率："+rAnalyzeAPIResult.getData().get(1)+"%");
                                } else {
                                    mBinding.txt2.setText("责任部门及时受理率："+"0.0%");
                                }
                                if (rAnalyzeAPIResult.getData().get(2)!=null) {
                                    mBinding.txt3.setText("按期办结率："+rAnalyzeAPIResult.getData().get(2)+"%");
                                } else {
                                    mBinding.txt3.setText("按期办结率："+"0.0%");
                                }
                                if (rAnalyzeAPIResult.getData().get(5)!=null) {
                                    mBinding.txt4.setText("信访部门满意率："+rAnalyzeAPIResult.getData().get(5)+"%");
                                } else {
                                    mBinding.txt4.setText("信访部门满意率："+"0.0%");
                                }
                                if (rAnalyzeAPIResult.getData().get(6)!=null) {
                                    mBinding.txt5.setText("责任部门满意率："+rAnalyzeAPIResult.getData().get(6)+"%");
                                } else {
                                    mBinding.txt5.setText("责任部门满意率："+"0.0%");
                                }
                                if (rAnalyzeAPIResult.getData().get(3)!=null) {
                                    mBinding.txt6.setText("信访部门参评率："+rAnalyzeAPIResult.getData().get(3)+"%");
                                } else {
                                    mBinding.txt6.setText("信访部门参评率："+"0.0%");
                                }
                                if (rAnalyzeAPIResult.getData().get(4)!=null) {
                                    mBinding.txt7.setText("网信占比率："+rAnalyzeAPIResult.getData().get(4)+"%");
                                } else {
                                    mBinding.txt7.setText("网信占比率："+"0.0%");
                                }
                            }
