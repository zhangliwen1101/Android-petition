...
public class PetitionStaffAnalyzeWangxinzhanbilvActivity extends PetitionStaffBaseActivity implements View.OnClickListener {
    ...
    // TODO: 2017/10/26 柱状图
    private String[] st={"总量","APP","微信","一屏通","网站"};
    // TODO: 2017/10/26 饼状图
    private String[] x = new String[] {"APP","微信","一屏通","网站"};
    private Float[] y;
    ...
    protected void initView() {
        ...
        mBinding= DataBindingUtil.setContentView(this, R.layout.petition_staff_activity_analyzexinfangbumenmanyilv);
        ...
        // barChart.invalidate();
        mBinding.txtTitle.setText("信访部门满意率");
   ...
   /* @SuppressWarnings("deprecation")
    ...
    private void GetAnalyze(String dataStart, String dataEnd, String type,String xingshi) {
        ...
        Subscription subscription = PetitionStaffApiService.Factory.create().GetWangxinzhanbiLv(GsonUtil.toJson(t))
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(Utils.mockSubscriber(this, Mock.PETITION_LOGIN,
                        new TypeToken<APIResult<RAnalyze>>() {
                            ...
                            public void onNext(APIResult<List<Float>> rAnalyzeAPIResult) {
                                ...
                                try {
                                    ...
                                    MpChartAndroidUtil.showBarChart1(PetitionStaffAnalyzeWangxinzhanbilvActivity.this, mBinding.barchart, barData);
                                } catch (Exception e) {

                                }
                                if (rAnalyzeAPIResult.getData().get(0)!=null) {
                                    mBinding.txt1.setText("纳入评价范围："+rAnalyzeAPIResult.getData().get(0));
                                } else {
                                    mBinding.txt1.setText("纳入评价范围："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(4)!=null) {
                                    mBinding.txt2.setText("已评价-小计："+rAnalyzeAPIResult.getData().get(4));
                                } else {
                                    mBinding.txt2.setText("已评价-小计："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(1)!=null) {
                                    mBinding.txt3.setText("已评价-满意："+rAnalyzeAPIResult.getData().get(1));
                                } else {
                                    mBinding.txt3.setText("已评价-满意："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(2)!=null) {
                                    mBinding.txt4.setText("已评价-基本满意："+rAnalyzeAPIResult.getData().get(2));
                                } else {
                                    mBinding.txt4.setText("已评价-基本满意："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(3)!=null) {
                                    mBinding.txt5.setText("已评价-不满意："+rAnalyzeAPIResult.getData().get(3));
                                } else {
                                    mBinding.txt5.setText("已评价-不满意："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(6)!=null) {
                                    mBinding.txt6.setText("超期未评价："+rAnalyzeAPIResult.getData().get(6));
                                } else {
                                    mBinding.txt6.setText("超期未评价："+"0.0%");
                                }
                                if (rAnalyzeAPIResult.getData().get(7)!=null) {
                                    mBinding.txt7.setText("待评价："+rAnalyzeAPIResult.getData().get(7));
                                } else {
                                    mBinding.txt7.setText("待评价："+"0.0%");
                                }

                                if (rAnalyzeAPIResult.getData().get(5)!=null) {
                                    mBinding.txt8.setText("满意率："+rAnalyzeAPIResult.getData().get(5)+"%");
                                } else {
                                    mBinding.txt8.setText("满意率："+"0.0%");
                                }
                            }
