        ...
        mBinding.listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (data1.size()==0) {
                    for (int i=0;i<list.size();i++) {
                        if (list.get(i).getDM().equals(getIntent().getExtras().getString("DM"))) {
                            editor.putString(getString(R.string.petition_staff_QY_SH),list.get(i).getMC());
                            editor.putString(getString(R.string.petition_staff_QY_SHDM),list.get(i).getDM());
                        }
                    }
                    editor.putString(getString(R.string.petition_staff_QY_S),data.get(position).getMC());
                    editor.putString(getString(R.string.petition_staff_QY_SDM),data.get(position).getDM().toString());

                    editor.putString(getString(R.string.petition_staff_QY_X),"");
                    editor.putString(getString(R.string.petition_staff_QY_XDM),"");

                    editor.commit();
                    PetitionStaffChangePersonalInfoAddress.instance.finish();
                    PetitionStaffChangePersonalInfoAddress01.this.finish();
                }
                mAdapter1=new AddAddressAdapter(data1,PetitionStaffChangePersonalInfoAddress01.this);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                for (int i=0;i<list1.size();i++) {
                    if (data1.get(position).getFJDM().equals(list1.get(i).getDM())) {
                        editor.putString(getString(R.string.petition_staff_QY_S),list1.get(i).getMC());
                        editor.putString(getString(R.string.petition_staff_QY_SDM),list1.get(i).getDM());
                    }
                }

                editor.putString(getString(R.string.petition_staff_QY_X),data1.get(position).getMC());
                editor.putString(getString(R.string.petition_staff_QY_XDM),data1.get(position).getDM().toString());
                editor.commit();
