...
public class PetitionStaffAnalyzeBumenmanyilvActivity02 extends PetitionStaffBaseActivity implements View.OnClickListener {
    ...
    private SpinnerStringAdapter arr_adapter01,arr_adapter03;
    private String type,ZeRenDanWei;
    @Override
        ...
        if (R.id.btn_query == id) {
            if (mBinding.startTime.getText().toString().equals("") || mBinding.endTime.getText().toString().equals("")) {
                ...
                if (getTime(mBinding.endTime.getText().toString()) < getTime(mBinding.startTime.getText().toString())) {
                    ...
                    GetAnalyze(mBinding.startTime.getText().toString(), mBinding.endTime.getText().toString(), type,ZeRenDanWei);
                }
            }
    ...
    protected void initView() {
        ...
        mBinding.llXingshi.setVisibility(View.GONE);
        //适配器
        ...
        // barChart.invalidate();
        mBinding.txtTitle.setText("网信占比率");
    ...
    private void GetAnalyze(String dataStart, String dataEnd, String type,String ZeRenDanWei) {
        ...
        Subscription subscription = PetitionStaffApiService.Factory.create().Getwangxinzhanbi(GsonUtil.toJson(t))
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(Utils.mockSubscriber(this, Mock.PETITION_LOGIN,
                        new TypeToken<APIResult<RAnalyze>>() {
                            ...
                            public void onNext(APIResult<List<Float>> rAnalyzeAPIResult) {
                                ...
                                try {
                                    ...
                                    MpChartAndroidUtil.showBarChart1(PetitionStaffAnalyzeBumenmanyilvActivity02.this, mBinding.barchart, barData);
                                } catch (Exception e) {

                                }
                                if (rAnalyzeAPIResult.getData().get(1)!=null) {
                                    mBinding.txt1.setText("总量："+rAnalyzeAPIResult.getData().get(1));
                                } else {
                                    mBinding.txt1.setText("总量："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(0)!=null) {
                                    mBinding.txt2.setText("网信信访量："+rAnalyzeAPIResult.getData().get(0));
                                } else {
                                    mBinding.txt2.setText("网信信访量："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(2)!=null) {
                                    mBinding.txt3.setText("网信占比率："+rAnalyzeAPIResult.getData().get(2)+"%");
                                } else {
                                    mBinding.txt3.setText("网信占比率："+"0");
                                }
                            }
