...
public class WorkWarningDetailOperateAdapter extends BaseAdapter {
    private Context mContext;
    private List<OperateInfo> data;

    public WorkWarningDetailOperateAdapter(List<OperateInfo> data, Context mContext) {
        this.data = data;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        View view;
        final WorkWarningDetailOperateAdapter.Holder holder;
        if (convertView!=null){
            view=convertView;
            holder= (WorkWarningDetailOperateAdapter.Holder) view.getTag();
        }else{
            view= View.inflate(mContext, R.layout.petition_staff_activity_work_warning_detail_list_item,null);
            holder=new WorkWarningDetailOperateAdapter.Holder();
        ...
        final OperateInfo mydata = data.get(position);
        //原先的正常数据的显示，操作等
        holder.txt1.setText(mydata.getJBJG());
        holder.txt2.setText(mydata.getBLFS());
        holder.txt3.setText(mydata.getQX());
        holder.txt4.setText(mydata.getJBR());
        holder.txt5.setText(mydata.getBLRQ());
        holder.txt6.setText(mydata.getBLZT());
        holder.txt7.setText(mydata.getBZ());
        holder.txt8.setText(mydata.getCLYJS());
        holder.txt9.setText(mydata.getCZFJ());
