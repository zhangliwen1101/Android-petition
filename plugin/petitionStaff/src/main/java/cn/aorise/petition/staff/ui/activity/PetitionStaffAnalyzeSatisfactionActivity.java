...
public class PetitionStaffAnalyzeSatisfactionActivity extends PetitionStaffBaseActivity implements View.OnClickListener {
    ...
    private String[] st={"全部信访件","已评价","未评价"};

    private String[] x = new String[] { "已评价","未评价" };
    private Float[] y;
    ...
    protected void initView() {
        ...
        mBinding.txtTitle.setText("按期办结率");
        // barChart.invalidate();
    ...
    private void GetAnalyze(String dataStart, String dataEnd, String type,String xingshi,String ZeRenDanWei) {
        ...
        Subscription subscription = PetitionStaffApiService.Factory.create().GetQunzhogncanpingLv(GsonUtil.toJson(t))
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(Utils.mockSubscriber(this, Mock.PETITION_LOGIN,
                        new TypeToken<APIResult<RAnalyze>>() {
                            ...
                            public void onNext(APIResult<List<Float>> rAnalyzeAPIResult) {
                                ...
                                try {
                                    ...
                                    MpChartAndroidUtil.showBarChart1(PetitionStaffAnalyzeSatisfactionActivity.this, mBinding.barchart, barData);
                                } catch (Exception e) {
                                ...
                                if (rAnalyzeAPIResult.getData().get(1)!=null) {
                                    mBinding.txt2.setText("已办结-小计："+rAnalyzeAPIResult.getData().get(1));
                                } else {
                                    mBinding.txt2.setText("已办结-小计："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(2)!=null) {
                                    mBinding.txt3.setText("按期办结件数："+rAnalyzeAPIResult.getData().get(2));
                                } else {
                                    mBinding.txt3.setText("按期办结件数："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(3)!=null) {
                                    mBinding.txt4.setText("超期办结件数："+rAnalyzeAPIResult.getData().get(3));
                                } else {
                                    mBinding.txt4.setText("超期办结件数："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(4)!=null) {
                                    mBinding.txt5.setText("超期未办结："+rAnalyzeAPIResult.getData().get(4));
                                } else {
                                    mBinding.txt5.setText("超期未办结："+"0");
                                }
                                if (rAnalyzeAPIResult.getData().get(5)!=null) {
                                    mBinding.txt6.setText("按期办结率："+rAnalyzeAPIResult.getData().get(5)+"%");
                                } else {
                                    mBinding.txt6.setText("按期办结率："+"0.0%");
                                }
                                if (rAnalyzeAPIResult.getData().get(6)!=null) {
                                    mBinding.txt7.setText("办结率："+rAnalyzeAPIResult.getData().get(6)+"%");
                                } else {
                                    mBinding.txt7.setText("办结率："+"0.0%");
                                }
                            }
