...
public class PetitionStaffImportantMonitorAddMatterAddressActivity112 extends PetitionStaffBaseActivity implements View.OnClickListener{
        ...
        mBinding.listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (data1.size()==0) {
                    for (int i=0;i<list.size();i++) {
                        if (list.get(i).getDM().equals(getIntent().getExtras().getString("DM"))) {
                            editor.putString(getString(R.string.petition_staff_contact_QY_SH11),list.get(i).getMC());
                            editor.putString(getString(R.string.petition_staff_contact_QY_SHDM11),list.get(i).getDM());
                        }
                    }
                    editor.putString(getString(R.string.petition_staff_contact_QY_S11),data.get(position).getMC());
                    editor.putString(getString(R.string.petition_staff_contact_QY_SDM11),data.get(position).getDM().toString());
                    ...
                    editor.commit();
                    PetitionStaffImportantMonitorAddMatterAddressActivity111.instance.finish();
                    PetitionStaffImportantMonitorAddMatterAddressActivity112.this.finish();
                }
                mAdapter1=new AddAddressAdapter(data1,PetitionStaffImportantMonitorAddMatterAddressActivity112.this);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                for (int i=0;i<list1.size();i++) {
                    if (data1.get(position).getFJDM().equals(list1.get(i).getDM())) {
                        editor.putString(getString(R.string.petition_staff_contact_QY_S11),list1.get(i).getMC());
                        editor.putString(getString(R.string.petition_staff_contact_QY_SDM11),list1.get(i).getDM());
                    }
                }

                editor.putString(getString(R.string.petition_staff_contact_QY_X11),data1.get(position).getMC());
                editor.putString(getString(R.string.petition_staff_contact_QY_XDM11),data1.get(position).getDM().toString());
                editor.commit();
